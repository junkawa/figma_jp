...
function translateMenu() {
    // メニューラベル要素の class
    const mainMenuLavelSelector = '.multilevel_dropdown--name--1abLT';
    const labels = document.querySelectorAll(mainMenuLavelSelector);
    // console.log(labels);
    labels.forEach(label => {
        const ret = mainMenuMap.find((m) => m["en"] === label.innerHTML); // 先頭一致
        if (ret) {
...
function observeMenu() {
    ...
    const mainMenuSelector = '.fullscreen_view--page--1QuyL';
    const mainMenuTarget = document.querySelector(mainMenuSelector);
    const config = {
        attributes: true,
        childList: true,
    ...
    // ページ読み込み完了待ち
    if (!mainMenuTarget) {
        window.setTimeout(observeMenu, 100);
        return;
    }

    const mainMenuObserver = new MutationObserver(function () {
        // 変更検知の無限ループ回避
        mainMenuObserver.disconnect();

        // メニュー生成検知時に翻訳する
        translateMenu();

        mainMenuObserver.observe(mainMenuTarget, config);
